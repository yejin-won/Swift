...
class ViewController: UIViewController {
    ...
    @IBAction func btnLampOff(_ sender: UIButton) {
        if isLampOn{
            ...
            let lampOnAlert = UIAlertController(title: "경고", message: "현재 Off 상태 입니다.", preferredStyle: .alert)
            let onAction = UIAlertAction(title: "네, 알겠습니다.", style: .default, handler: nil)
            lampOnAlert.addAction(onAction)
            present(lampOnAlert, animated: true, completion: nil)
    ...
    @IBAction func btnLampRemove(_ sender: UIButton) {
        ...
        let removeAction = UIAlertAction(title: "네, 제거 합니다.", style: .default, handler: {ACTION in
